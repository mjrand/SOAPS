import time
from scpi_psu_driver import PSUInterface
...
class Channel:
    ...
    def enable_output(self):
        ...
        self.parent_session.setOutput(self.channel_number, state=True)

    def disable_output(self):
        self.output_enabled = False

        self.parent_session.setOutput(self.channel_number, state=False)

    def set_voltage(self, new_voltage):
 
        self.parent_session.setVolt(new_voltage)

    def get_voltage(self):
        channel_voltage = self.parent_session.getVolt(self.channel_number)

        return channel_voltage

    def get_current(self):
        channel_current = self.parent_session.getCurr(self.channel_number)
...
class PowerSupply:
    ...
    def connect_to_power_supply(self, power_supply_name):
        ...
        if power_supply_name == "Keithley":
            ...
            gpibAddr = 10
            new_session = psuInterface(ip_address, gpibAddr)
            self.name = "Keithley"
